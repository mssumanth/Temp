...
class CreateNetwork(forms.SelfHandlingForm):
    ...
    failure_url= 'horizon:project:vpns:index'
...
class AddSite(forms.SelfHandlingForm):
    ...
    def handle(self, request, data):
        ...
        match = re.search('/project/vpns/([^/]+)/addsite', uri)
...
class GetBranchGwVM(forms.SelfHandlingForm):
    ...
    def handle(self, request, data):
        print "+++++++RBA RBA RBA +++++ ////////////// data = %s"%data
        uri = request.get_full_path()
        match = re.search('/project/vpns/([^/]+)/getbranchgwvm', uri)
        vpn_id = match.group(1)
        try:
...
class AddGW(forms.SelfHandlingForm):
    ...
    def handle(self, request, data):
        print "++++++++++++ ////////////// data = %s"%data
        uri = request.get_full_path()
        match = re.search('/project/vpns/([^/]+)/addgw/', uri)
...
class CreateGateway(forms.SelfHandlingForm):
    ...
    gwchoice = forms.ChoiceField(required=True, label='Add a Gateway', choices=GCHOICES)
    print "******** sitechoice=%s"%sitechoice
    success_url= 'horizon:project:vpns:creategw'
    failure_url= 'horizon:project:vpns:creategw'

    def get_success_url(self):
        return reverse("horizon:project:vpns:creategw")

    def handle(self, request, data):
        print "++++++++++++ ////////////// data = %s"%data
        uri = request.get_full_path()
        match = re.search('/project/vpns/([^/]+)/creategw/', uri)
        elasticnet_id = match.group(1)
        try:
            if data['sitechoice'] in ['new', '']:
            ...
            exceptions.handle(request, _('Unable to add gateway.'))
        return shortcuts.redirect("horizon:project:vpns:creategw")
...
class PreAddLink(forms.SelfHandlingForm):
    ...
    p_site=''
    e_site=''
    success_url= 'horizon:project:vpns:addlink'
    failure_url= 'horizon:project:vpns:index'

    def get_success_url(self):
        return reverse("horizon:project:vpns:addlink")
	...
	# get vpn_id
        uri = request.get_full_path()
        match = re.search('/project/vpns/([^/]+)/preaddlink/', uri)
        vpn_id = match.group(1)
