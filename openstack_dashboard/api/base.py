...
def get_service_from_catalog(catalog, service_type):
    ...
    #if service_type == 'vpns':
    #	return service_type
    if catalog:
        for service in catalog:
	    print "Service inside get_service_from_catalog:", service
            if 'type' not in service:
                continue
            if service['type'] == service_type:
		print "The service being returned is: ",service
                return service
    return None
...
def get_url_for_service(service, region, endpoint_type):
    if 'type' not in service and service != 'vpns':
        return None

    identity_version = get_version_from_service(service)
    print "Identity Version: ", identity_version
    service_endpoints = service.get('endpoints', [])
    print "Service Endpoints: ", service_endpoints
    ...
    for endpoint in available_endpoints:
        try:
		    ...
		    print "RET URL:",endpoint.get('url')
                    return endpoint.get('url')
		print "Sumanth here at endpoint"
...
def url_for(request, service_type, endpoint_type=None, region=None):
    endpoint_type = endpoint_type or getattr(settings,
                                             ...
                                             'publicURL')
    print "ENDPOINT: ",endpoint_type
    fallback_endpoint_type = getattr(settings, 'SECONDARY_ENDPOINT_TYPE', None)
    print "FALLBACK: ",fallback_endpoint_type
    print "REQUEST in API-base.py: ", request
    catalog = request.user.service_catalog
    print "Catalog: ",catalog
    service = get_service_from_catalog(catalog, service_type)
    print "BASE::Service: ",service
    print "BASE::Request: ",request
    if service == 'vpns':
	url = 'horizon:project:vpns:index'
	...
	print "URL: ",url
	    ...
	    print "URL present",url
            return url
    print "Sumanth here at no service"
